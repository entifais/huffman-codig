...
string read_file(string name){
    ...
    using Iterator = istreambuf_iterator<char>;
    string content(Iterator{file}, Iterator{});
    if(!file)
        throw runtime_error("Failed to read " + name);
    return content;
}
string read_file_bin(string name){
...
void compres_file(string name,string data){
...
int main(){
    ...
    cout<<endl<<"Canonico vs Simbolo"<<endl;
    ...
    cout<<endl<<"Texto decodificado (Arhivo out.bin)"<<decoded_text<<endl;
    //string data=read_file_bin("out.bin");
    //cout<<data;
    return 0; 
